...
x_cols = D.iloc[:, -32:].columns.tolist()
event_col=['label']
time_col = ['value']
...
for i in range(n_exp):
    ...
    df_train, df_val, df_test = train_val_test_stratified_split(d, 'label', frac_train=0.80, frac_val=0.05, frac_test=0.15, random_state=10)
    ...
    get_target = lambda df: (df['value'].values, df['label'].values)
    ...
    surv_df.index.name = 'value'
    surv_df.columns.name = 'survival_function'

    surv_df.to_csv("./output/discharge_deephits.csv", index=True)

    survival_predictions = pd.Series(trapz(surv.values.T, surv.index), index=df_test.index)
    l1_hinge = l1(df_test.value, df_test.label, survival_predictions, l1_type = 'hinge')
    l1_margin = l1(df_test.value, df_test.label, survival_predictions, df_train.value, df_train.label, l1_type = 'margin')
    ...
    #_ = plt.xlabel('value')
    quantiles = np.percentile(df_test['value'], np.linspace(1, 99, 362))
    # quantiles = np.sort(df_test['value'].unique())

    print(f"Selected Eval Times: {quantiles[:5]} ... {quantiles[-5:]}")

    events_train = np.array([(e, t) for e, t in zip(df_train['label'], df_train['value'])], dtype=[('label', 'bool'), ('value', 'float')])
    events_test = np.array([(e, t) for e, t in zip(df_test['label'], df_test['value'])], dtype=[('label', 'bool'), ('value', 'float')])

    time_grid_train = np.unique(df_train['value'])
    ...
    for eval_time in quantiles:  
        ...
        try:
            ...
            auc = cumulative_dynamic_auc(events_train, events_test, estimated_risks, times=[eval_time])[0][0]
...
#print(one_calibration(df_test.time, df_test.label, surv.iloc[6], time = 40))
print('d_calibration_p_value:', round(d_calibration(df_test.label, surv.iloc[6])['p_value'], 3))
print('D-Calibration:', round(sum(d_calibration(df_test.label, surv.iloc[6])['bin_proportions']), 3))
print('d_calibration_bin_proportions:')
for i in d_calibration(df_test.label, surv.iloc[6])['bin_proportions']:
    print(i)
print('D-Calibration_censored:', round(sum(d_calibration(df_test.label, surv.iloc[6])['censored_contributions']), 3))
print('d_calibration_censored_contributions:')
for i in d_calibration(df_test.label, surv.iloc[6])['censored_contributions']:
    print(i)
print('D-Calibration_uncensored:', round(sum(d_calibration(df_test.label, surv.iloc[6])['uncensored_contributions']), 3))
print('d_calibration_uncensored_contributions:')
for i in d_calibration(df_test.label, surv.iloc[6])['uncensored_contributions']:
    print(i)